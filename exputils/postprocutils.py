...
def postprocess(
    ...
    if not isinstance(out_file, h5py.Group):
        raise TypeError(f"`out_file` must be instance of `h5py.Group` not {type(out_file)}")
...
def postprocess_parallel(
        ...
        get_result_to_proc: Callable[[BaseTransientModel, str], Mapping[str, Callable[[sf.StateFile], np.ndarray]]],
        overwrite_results: Optional[Container[str]] = None,
        use_tqdm: Optional[bool]=True,
        num_proc: Optional[int]=1
    ):
    ...
    if num_proc == 1:
        with sf.StateFile(model, in_file, mode='r') as f:
            results = [
                result_to_proc[result_name](f) for result_name in result_names
            ]
    else:
    ...
    case_to_signal_update = {
        ...
        for result_name, result in zip(result_names, results)
    }
